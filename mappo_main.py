...
BASE_KEY = 'proprio'
class AIJ_Wrapper(JaxMARLWrapper):
    ...
    def world_state(self, obs, state):
        ...
        """
        return jnp.array([obs[agent][BASE_KEY] for agent in self._env.agents])
        # hands = state.player_hands.reshape((self._env.num_agents, -1))
        # return jnp.concatenate((all_obs, hands), axis=1)
        
    @partial(jax.jit, static_argnums=0)
    def world_state_size(self):
        return self._env.observation_space(self._env.agents[0])[BASE_KEY].shape[0] * self._env.num_agents # + 125 # NOTE hardcoded hand size
...
def make_train(config):
    ...
    def train(rng):
        ...
        ac_init_x = (
            jnp.zeros((1, config["NUM_ENVS"], *env.observation_space(env.agents[0])[BASE_KEY].shape)),
        ...
        cr_init_x = (
            jnp.zeros((1, config["NUM_ENVS"], 56)), # NOTE hardcoded
            jnp.zeros((1, config["NUM_ENVS"])),
        )
        ...
        obsv, env_state = env.reset(config['SEED'])
        # obsv, env_state = jax.vmap(env.reset, in_axes=(0,))(reset_rng[0])
...
@hydra.main(version_base=None, config_path="", config_name="mappo_config")
def main(config):
